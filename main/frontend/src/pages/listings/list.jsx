    ...
    <div id="main">
      <div className="content">
        <header>
          ...
          {role === "super_user" && <Link to='/listings/new'>Create new listing</Link>}
        </header>
            ...
            (<ul className="api-data">
              {data?.map((item, key) => (
                <li key={key}>
                  ...
                  <h3><span>from:</span> {new Date(item?.start_at).toString().slice(0,10).replace(/-/g,"")}</h3>
                  <h3><span>to:</span> {new Date(item?.end_at).toString().slice(0,10).replace(/-/g,"")}</h3>
                  <div className="actions">
                    <Button onClick={() => handleRedirects(`listings/${item?.id}`)} success content="view"/>
                    {role === 'super_user' &&
                    <Button onClick={() => handleRedirects(`listings/${item?.id}/edit`)} warning content="edit"/>}
                    {role === 'super_user' && <Button onClick={() => handleListingDelete(item?.id)} danger content="delete"/>}
                  </div>
                </li>
              ))}
            </ul>)
